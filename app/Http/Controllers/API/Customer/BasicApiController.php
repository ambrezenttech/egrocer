...
class BasicApiController extends Controller
{
    ...
    public function getSellersByCategory(Request $request)
    {
        ...
        $sellers = Seller::whereIn('sellers.id', $seller_ids)->select('sellers.id', 'sellers.name', 'sellers.street', 'sellers.store_name', 'sellers.logo', DB::raw("ROUND(6371 * acos(cos(radians(" . $request->latitude . "))
        ...
        if (count($sellers) > 0) {
            return response()->json(compact('sellers', 'seller_ids'));
        } else {
            return CommonHelper::responseError(__('seller_not_found'));
        }
    }
